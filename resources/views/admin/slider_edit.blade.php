    ...
    <div class="container-fluid">
        ...
        <div class="row">
            <div class="col-12 col-sm-12">
                    ...
                    <form method="post" action="{{ route('updateSlider') }}" id="updateProfileForm" enctype="multipart/form-data">
                        @csrf
                        <div class="row">
                            <input type="hidden" name="type" value="slider" class="form-control col-sm-3"/>
                            <div class="col-sm-12">
                                <a type="submit" onclick="AjaxOnline()" class="btn btn-info">Translate</a>
                            </div>
                            <div class="col-sm-4">
                                <strong> Header in russian</strong>
                                            <textarea onclick='spaceDel(id)' id="header_ru" name="header[ru]" maxlength="255" class="form-control">{{ $info->head_ru }}</textarea>
                            </div>
                            <div class="col-sm-4">
                                <strong> Header in Uzbek</strong>
                                            <textarea id="header_uz" onclick='spaceDel(id)' name="header[uz]" maxlength="255" class="form-control">{{ $info->head_uz }}</textarea>

                            </div>
                            <div class="col-sm-4">
                                <strong> Header in English</strong>
                                            <textarea id="header_en" name="header[en]" onclick='spaceDel(id)' maxlength="255" class="form-control">{{ $info->head_en }}</textarea>
                            </div>
                            <div class="col-sm-4">
                                <strong> Short text in russian</strong>
                                            <textarea id="short_ru" name="short[ru]" onclick='spaceDel(id)' maxlength="255" class="form-control">{{ $info->short_ru }}</textarea>
                            </div>
                            <div class="col-sm-4">
                                <strong> Short text in Uzbek</strong>
                                            <textarea id="short_uz" name="short[uz]" onclick='spaceDel(id)' maxlength="255" class="form-control">{{ $info->short_uz }}</textarea>

                            </div>
                            <div class="col-sm-4">
                                <strong> Short text in English</strong>
                                            <textarea id="short_en" name="short[en]" onclick='spaceDel(id)' maxlength="255" class="form-control">{{ $info->short_en }}</textarea>
                            </div>

                            <div class="col-sm-4">
                                <strong> Full text in russian</strong>
                                            <textarea onclick='spaceDel(id)' id="full_ru" name="full[ru]" class="form-control">{{ $info->full_ru }}</textarea>
                            </div>
                            <div class="col-sm-4">
                                <strong> Full text in Uzbek</strong>
                                            <textarea id="full_uz" name="full[uz]" onclick='spaceDel(id)' class="form-control">{{ $info->full_uz }}</textarea>
                            </div>
                            <div class="col-sm-4">
                                <strong> Full text in English</strong>
                                            <textarea onclick='spaceDel(id)' id="full_en" name="full[en]" class="form-control">{{ $info->full_en }}</textarea>
                            </div>
                                ...
                                <div style="border:1px #ccc solid;min-height: 100px;width: 100%" id="gal_div_org" class="flexs">
                                    ...
                                    @for($i=0;$i<count($info->media);$i++)
                                        ...
                                        <a data-toggle="modal" onclick="relbacksExist(this)">
                                            <div class="gal">
                                                <img  id="modal_img1_{{$info->media[$i]}}" src="{{ URL::to('/') }}/media/{{$info->media[$i]}}" alt="">
                                                <p>{{$info->media[$i]}}</p>
