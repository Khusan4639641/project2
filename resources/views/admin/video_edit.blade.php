    ...
    <div class="container-fluid">
        ...
        <div class="row">
            <div class="col-12 col-sm-12">
                    ...
                    <form method="post" action="{{ route('updateVideo') }}" id="updateProfileForm" enctype="multipart/form-data">
                        @csrf
                        <div class="row">
                                ...
                                <div style="border:1px #ccc solid;min-height: 100px;width: 100%" id="gal_div_org" class="flexs">
                                    ...
                                    @for($i=0;$i<count($info->media);$i++)
                                        ...
                                        <a data-toggle="modal" onclick="relbacksExist(this)">
                                            <div class="gal">
                                                <video  id="modal_img1_{{$info->media[$i]}}" src="{{ URL::to('/') }}/media/{{$info->media[$i]}}" alt="" controls></video>
                                                <p>{{$info->media[$i]}}</p>
            ...
            <div class="modal-body flexing" id="exist_image">
                @foreach($file_image as $val)
                    <div class="gal" onclick="choise(id)" id="gal_{{ $val->img_url }}">
                        <video id="img_gal_{{ $val->img_url }}" src="{{ URL::to('/') }}/media/{{ $val->img_url }}" alt="" controls></video>
...
//        alert(id);
    ...
    function uploadUserProfileImage(){
        ...
        $.ajax({
            ...
            success:function(response){
                ...
                video.setAttribute('src',window.location.origin+"/media/"+file_name);
                video.setAttribute('id',"img_gal_"+file_name);
    ...
    function choise(id){
        ...
        var video=document.createElement("video");
        video.setAttribute('src',get_image);
